...
{
    if (argc != 8) {
        fprintf(stderr, "multipart-upload-part <bucket> <local-file> <object> <upload-id> <position> <size> <part-number>\n");
        exit(1);
    }
    bucket_name = argv[1];
    ...
    pos = aos_atoi64(pos_ptr);
    size = aos_atoi64(size_ptr);
    ...
    assert(pos < file_length && pos >= 0);
    upload_file->file_pos = pos;
    upload_file->file_last = (pos + size < file_length ? pos + size : file_length);
